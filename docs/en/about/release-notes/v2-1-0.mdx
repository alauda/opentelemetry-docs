...
Upstream renamed most component type identifiers to snake_case. The previous names remain available as deprecated aliases, so existing configurations continue to work, but the Collector logs a deprecation warning on startup and upstream intends to remove the aliases in a future release. Update your `OpenTelemetryCollector` resources to the new names, with the exception noted below.
...
:::warning Components that drive automatic RBAC generation are an exception
The Operator matches component type names literally when it decides which RBAC to generate, and it has not yet been updated for all of the new names. Keep configuring the Kubernetes Objects receiver as `k8sobjects`, the Kubelet Stats receiver as `kubeletstats`, and the Resource Detection processor as `resourcedetection`. Under their snake_case names, the Operator generates no `ClusterRole` for these components and the Collector fails at runtime with permission errors; `kubelet_stats` additionally loses the injected `K8S_NODE_NAME` environment variable.

The Kubernetes Attributes processor is safe to rename — the Operator recognizes both `k8sattributes` and `k8s_attributes`. See [Creating the Required RBAC Resources Automatically](../../installing/rbac-resources.mdx#component-name-spelling).
:::
