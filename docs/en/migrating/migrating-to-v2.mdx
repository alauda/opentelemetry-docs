...
3. Delete the RBAC resources that the v1 deployment created in `cpaas-system` (skip this step if your v1 deployment did not create them):

TODO: 将下面删除的资源添加到 #back-up-v1-resources 备份步骤中，用于后续回滚。
...
For each `Instrumentation` resource that you backed up, recreate it on v2 with the new `spec.java.image` field set. The exporter endpoint and other environment variables follow the same shape used in v1. Update the value if the Collector namespace or service name has changed.

TODO: 说明 autoinstrumentation-java 2.x 版本，otel 默认导出已经从 grpc 改为 http/protobuf 了，需要将 otel collector 端口从 4317 改为 4318。
...
TODO: 补充一条说明，使用命令，重启全部注入了 java agent 的 deployment。
...
TODO: Pod 可能并没不能执行 env 命令，将其优化为查看 pod 的 yaml。
