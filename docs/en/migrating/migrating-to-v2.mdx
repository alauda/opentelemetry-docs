...
- The `yq` command-line YAML processor, version 4, is installed if you use the backup conversion examples in this guide.
...
2. Recreate the `OpenTelemetryCollector` resource based on your backup, ensuring that every component used in `spec.config` is supported on v2.
    ...
    For other scenarios, follow [Deploying the OpenTelemetry Collector](../installing/install-opentelemetry.mdx#deploying-the-opentelemetry-collector) and adapt the example to your environment. The following example copies `./otel-v1-backup/collectors.yaml` into a new working directory, removes fields that cannot be reused on create, converts the old `service.telemetry.metrics.address` field to the current `service.telemetry.metrics.readers[].pull.exporter.prometheus` shape described in the [OpenTelemetry Collector internal telemetry documentation](https://opentelemetry.io/docs/collector/internal-telemetry/#prometheus-endpoint-for-internal-metrics), and creates the Collector resources:
    ...
    yq eval -i '
      ...
      | .items[] |= (
          ...
          | with(.spec.config.service.telemetry.metrics;
              del(.address)
              | .readers = [
                  {
                    "pull": {
    ...
    ' "$RESTORE_DIR/collectors.yaml"

    kubectl apply -f "$RESTORE_DIR/collectors.yaml"
    ```
...
JAVA_AUTO_INSTRUMENTATION_IMAGE="$JAVA_AUTO_INSTRUMENTATION_IMAGE" yq eval -i '
  del(.metadata)
  | .items[] |= (
      del(
        .metadata.annotations."kubectl.kubernetes.io/last-applied-configuration",
      ...
      | .spec.java.image = strenv(JAVA_AUTO_INSTRUMENTATION_IMAGE)
      | (.spec.env[] | select(.name == "OTEL_EXPORTER_OTLP_ENDPOINT").value) |= sub(":4317", ":4318")
      | (select(.spec.java.env) | .spec.java.env[] | select(.name == "OTEL_EXPORTER_OTLP_ENDPOINT").value) |= sub(":4317", ":4318")
    )
' "$RESTORE_DIR/instrumentations.yaml"

kubectl apply -f "$RESTORE_DIR/instrumentations.yaml"
```
