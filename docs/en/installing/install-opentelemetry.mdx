...
### Installing via the web console \{#installing-via-the-web-console}
...
### Installing via the CLI \{#installing-via-the-cli}
...
1. Check available versions
    ...
    **Example output**
    ...
    stable   opentelemetry-operator2.v0.156.0-rc.2     0.156.0-rc.2
    ```
...
3. Create a namespace
    ...
    ```bash {name=install-otel:create-namespace-opentelemetry-operator2}
    kubectl get namespace opentelemetry-operator2 || kubectl create namespace opentelemetry-operator2
    ```

4. Create a Subscription

    ```bash {name=install-otel:create-subscription-opentelemetry-operator2}
    kubectl apply -f - <<EOF
    ...
    spec:
      ...
      startingCSV: opentelemetry-operator2.v0.156.0-rc.2
    EOF
    ```
...
6. Approve InstallPlan
    ...
    **Example output**
    ...
    install-abc12   opentelemetry-operator2.v0.156.0-rc.2     Manual     false
    ```

    **Approve manually**

    ```bash {name=install-otel:approve-installplan-manual}
...
opentelemetry-operator2.v0.156.0-rc.2     Alauda Build of OpenTelemetry v2         0.156.0-rc.2              Succeeded
```
...
:::
  ...
  ```bash {name=install-otel:create-namespace-opentelemetry-collector}
...
1. Customize and apply the `OpenTelemetryCollector` custom resource (CR):
    ...
    ```bash {name=install-otel:deploy-collector}
    kubectl apply -f - <<EOF
    apiVersion: opentelemetry.io/v1beta1
    kind: OpenTelemetryCollector
    metadata:
...
```shell {name=install-otel:check-collector-pods}
kubectl get pods -n opentelemetry-collector
```

**Example output**

```text {name=install-otel:check-collector-pods-output}
...
To remove the Alauda Distributed Tracing components from your cluster, follow [Uninstalling Alauda Build of OpenTelemetry v2](../uninstalling/uninstalling-opentelemetry.mdx).
