...
The automatic path is also blocked for Java in this release: the recorded default moves from 1.33.6 to 2.30.0, a major-version jump that the Operator refuses to apply on its own. It reports `upgrade blocked for unupgradable instrumentation version` in its log and leaves the resource unchanged.
:::
