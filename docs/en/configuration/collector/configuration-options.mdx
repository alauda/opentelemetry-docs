...
<Callouts>
1. The OTLP **exporter** is named `otlp_grpc`, while the OTLP **receiver** above keeps the name `otlp`. See [Component type names](#component-type-names).
...
## Component type names \{#component-type-names}
...
| Component | Deprecated type name | Current type name |
| :-------- | :------------------- | :---------------- |
...
For the full list of renamed components, see the [v2.1.0 Release Notes](../../about/release-notes/v2-1-0.mdx#snake-case-names).
...
| `exporters` | Defines destinations for processed data. At least one exporter must be enabled in a pipeline for valid configuration. | otlp_grpc, otlp_http, debug, prometheus, kafka | None |
