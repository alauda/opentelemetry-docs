  ...
  java:
    ...
    env:
      - name: OTEL_JAVAAGENT_DEBUG
        value: "false"
  exporter:
    endpoint: http://otel-collector.observability.svc:4318 # [!code callout]
  env:
...
<Callouts>
1. The Java auto-instrumentation image used by the Operator to inject the agent. The v2 Operator does not ship a Java agent image, so you must build or supply one yourself. See the warning in [Instrumentation Options](./instrumentation-options.mdx).
2. The OTLP exporter endpoint. Replace this URL with the address of your actual OpenTelemetry Collector.
3. Enables Java auto-instrumentation injection for this pod. See [Supported Values for Injection Annotations](./instrumentation-options.mdx#supported-values-for-injection-annotations) for the accepted values.
</Callouts>

## Correlating Application Logs with Traces

Once the Java agent is injected, it can also add the active `trace_id` and `span_id` to your application logs, letting you pivot between a log line and its trace. For Spring Boot, Logback, and Log4j 2 examples, see [Correlating Application Logs with Traces](./correlating-logs-with-traces.mdx).
