...
Uninstalling the Alauda Build of OpenTelemetry v2 Operator from an Alauda Container Platform involves deleting the following components:
  ...
  - `opentelemetry-operator2` namespace
...
### Delete the `opentelemetry-operator2` namespace

    ```shell
    kubectl delete ns opentelemetry-operator2
    ```
...
2.  Use the namespace and name from the previous command's output to replace `<namespace>` and `<name>`, and then run the following to remove each resource:
    ...
    **Example output**
    ...
    instrumentation.opentelemetry.io "my-instrumentation" deleted
    ```
...
1.  Get the `OpenTelemetryCollector` resources by executing this command:
    ...
    **Example output**
    ...
    NAMESPACE   NAME   MODE         VERSION   READY   AGE
    default     otel   deployment   0.146.0   True    1d
    ```
...
1.  Delete the `opentelemetry-operator2` namespace with the following command:

    ```shell {name=uninstall-otel:delete-namespace}
    kubectl delete ns opentelemetry-operator2
    ```

    **Example output**

    ```shell {name=uninstall-otel:delete-namespace-output}
    namespace "opentelemetry-operator2" deleted
    ```
...
1.  Delete the `opentelemetry-operator2` subscription:
    ...
    **Example output**
    ...
    subscription.operators.coreos.com "opentelemetry-operator2" deleted from opentelemetry-operator2 namespace
    ```
